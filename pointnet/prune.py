...
def load_weights(model1,model2, bias = False):
    ...
    with torch.no_grad():
        for (name1, module1), (name2, module2) in zip(model1.named_modules(), model2.named_modules()): 
            if isinstance(module1, nn.Linear) or  isinstance(module1, nn.Conv2d) or isinstance(module1, nn.BatchNorm2d) or isinstance(module1, nn.Conv1d):
...
def prune_rate_calculator(model, bias=True):
    ...
    for name, module in model.named_modules():
        if isinstance(module, nn.Linear) or  isinstance(module, nn.Conv2d) or isinstance(module, nn.BatchNorm2d) or isinstance(module, nn.Conv1d):
