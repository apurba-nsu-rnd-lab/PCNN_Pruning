...
def weight_rewinding(pruned_model, unpruned_model, bias=True):
    ...
    with torch.no_grad():
        for pruned_module, unpruned_module in zip(pruned_model.modules(), unpruned_model.modules()):
            if isinstance(pruned_module, nn.Linear) or  isinstance(pruned_module, nn.Conv2d) or isinstance(pruned_module, nn.Conv1d) or isinstance(pruned_module, nn.BatchNorm2d):
...
def print_nonzeros(model, bias=True):
    ...
    for name, module in model.named_modules():
        if isinstance(module, nn.Linear) or  isinstance(module, nn.Conv2d) or isinstance(module, nn.Conv1d) or isinstance(module, nn.BatchNorm2d):
            from prune import check_pruned
